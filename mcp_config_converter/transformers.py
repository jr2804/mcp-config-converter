...
class ConfigTransformer:
    ...
    @classmethod
    def parse_file(cls, file_path: str | Path) -> MCPConfig:
    ...
    def format_config(cls, config: MCPConfig, provider: str) -> str:
        ...
        if provider not in cls.FORMATTERS:
            raise ValueError(f"Unsupported provider: {provider}. Supported: {list(cls.FORMATTERS.keys())}")
    ...
    def transform(
        ...
        result = cls.format_config(config, provider) if provider else config.model_dump_json(indent=2)
